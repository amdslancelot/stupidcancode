...
public class Candy {
    ...
    public static void main(String[] args) {
        ...
        for(int[] e : cases) {
            int r = test.candy(e);
            int a = answers[n];
            boolean success = (a == r) ? true : false;
            if(!success) {
                System.out.printf("### ERROR ### case%d, r=%d, a=%d, %s\n", n, r, answers[n], (a == r) ? true : false);
            }
            System.out.println();
            n++;
        }
    ...
    public int candy(int[] ratings) {
        System.out.println("Array: " + Arrays.toString(ratings));
        int rl = ratings.length;
        int[] nums = new int[rl];
        int n = 1, total = 0, minIndex = 0, minValue = 0, smallerStart = 0;
        ...
        System.out.printf("minIndex=%d, minValue=%d\n", minIndex, minValue);

        /* Dealing with the right side of the min value */
        for(int i=minIndex+1; i<rl; i++) {
            if(ratings[i] > ratings[i-1]) {
                ...
                n = 1;
                nums[i] = n;
                if(i-2 == -1 || ratings[i-2] <= ratings[i-1]) {
                    /* If the element before the previous element:
                       ( is the left edge || is the smaller beginning ) */
                    smallerStart = i-1; /* Make the previous element the smaller beginning */
                }
                //System.out.printf("i=%d, smallerStart=%d\n", i, smallerStart);
                for(int j=i-1; j>=smallerStart; j--) {
                    if(j == smallerStart && nums[j] == nums[j-1] + 1 && nums[j] > nums[j+1]) {
                        // If jth element is the smaller start && already has more candy than its neighbors, do nothing
                        ;
                    } else {
                        int t = nums[j];
                        nums[j] = nums[j+1] + 1;
                        total = total + ( nums[j] - t );
                    }
                }
            } else {
                /* Case: Equals. Make it 1 for now. */
                n = 1;
                nums[i] = n;
            }
            total = total + n;
        ...
        n = 1;
        /* Dealing with the left side of the min value */
        for(int i=minIndex-1; i>=0; i--) {
            if(ratings[i] > ratings[i+1]) {
                ...
                n = 1;
                nums[i] = n;
                if(i+2 == rl || ratings[i+2] <= ratings[i+1]) { 
                    /* If the element before the previous element: 
                       ( is the right edge || is the smaller beginning ) */
                    smallerStart = i+1; /* Make the previous element the smaller beginning */
                }
                //System.out.printf("i=%d, smallerStart=%d\n", i, smallerStart);

                /* Looking back from the smaller start, give 1 more candy each */
                for(int j=i+1; j<=smallerStart; j++) {
                    if(j == smallerStart && nums[j] == nums[j+1] + 1 && nums[j] > nums[j-1] ) {
                        // If jth element is the smaller start && already has more candy than its neighbors, do nothing
                        ;
